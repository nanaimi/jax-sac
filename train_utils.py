...
def do_episode(agent, training, environment, config, pbar, render):
    ...
    while not (termination or truncation):
        ...
        next_observation, reward, termination, truncation, info = environment.step(
            action
        )
        terminal = termination and not truncation
        if training:
            agent.observe(
                dict(
...
def train(config, agent, environment, logger):
    ...
    if pathlib.Path(config.log_dir, "agent_data").exists():
        ...
        print(
            "Loaded {} steps. Continuing training from {}".format(steps, config.log_dir)
        )
