...
class SAC:
    def __init__(
        ...
        self.entropy_bonus = utils.Learner(
            lambda log_pi: EntropyBonus()(log_pi),
            next(self.rng_seq),
            config.alpha_opt,
            True,
            np.ones((1,)),
        ...
        self._prefill_policy = lambda: np.random.uniform(
    ...
    def __call__(self, observation: Observation, training=True):
        ...
        if self.time_to_clone_critics:
            self.target_critics.params = utils.clone_model(
                self.critics.params, self.target_critics.params, self.config.tau
            )

        action = self.policy(
            observation, self.actor.params, next(self.rng_seq), training
        )
    ...
    def policy(
        ...
        action = policy.sample(seed=rng_key) if training else policy.mode(seed=rng_key)
        return action
    ...
    def update_actor_critic(self, batch):
        ...
        self.actor.params, self.actor.opt_state, actor_report = self._update_actor(
            self.actor.params,
            self.critics.params,
            self.entropy_bonus.params,
            next(self.rng_seq),
            self.actor.opt_state,
            batch,
        )
        (
            self.entropy_bonus.params,
    ...
    def _update_critics(
        ...
        def loss(
            ...
            next_qs = self.target_critics.apply(
                target_critic_params, batch["next_observation"], next_action
            )
            ...
            soft_q_target = utils.td_error(
                soft_q,
                batch["reward"] * self.config.reward_scale,
                batch["terminal"],
                self.config.discount,
            )
            qs = self.critics.apply(
            ...
            critics_loss = sum(
                (
                    map(
                        lambda q: -jnp.mean(
                            q.log_prob(jax.lax.stop_gradient(soft_q_target))
                        ),
                        qs,
                    )
                )
        ...
        grads, report = jax.grad(loss, has_aux=True)(
            critic_params, target_critic_params, actor_params, entropy_params, rng_key
        )
    ...
    def _update_actor(
        ...
        def loss(
            ...
            entropy_bonus = self.entropy_bonus.apply(
                entropy_params, policy.log_prob(action)
            )
            actor_loss = -jnp.mean(debiased_q + entropy_bonus)
            return actor_loss, {"agent/actor/loss": actor_loss}

        grads, report = jax.grad(loss, has_aux=True)(
            actor_params, critic_params, entropy_params, rng_key
        )
    ...
    def _update_alpha(
        ...
        updates, new_opt_state = self.entropy_bonus.optimizer.update(grads, opt_state)
        new_params = optax.apply_updates(entropy_params, updates)
        report.update(
            {
                "agent/alpha/grads": optax.global_norm(grads),
            ...
            cloudpickle.dump(
                {
                    ...
                    "critics_params": self.critics.params,
                    "entropy_params": self.entropy_bonus,
    ...
    def time_to_update(self):
        return (
            self.training_step > self.config.prefill
            and self.training_step % self.config.train_every == 0
        )
    ...
    def time_to_clone_critics(self):
        return (
            self.training_step > self.config.prefill
            and self.training_step % self.config.slow_target_update == 0
        )
